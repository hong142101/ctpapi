...
{
private:
	CTPAPI * ctpapi = nullptr;
	CThostFtdcMdApi * api = nullptr;
	std::atomic_bool frontconnected = false;
	std::atomic_bool userLogined = false;

public:
	explicit MdApi(CTPAPI * ctpapi) { this->ctpapi = ctpapi; }
	~MdApi() {}
	//---------------------------------------
	// ctp api 回调函数
	//---------------------------------------
...
private:
	...
	std::atomic_bool frontconnected = false;
	std::atomic_bool userLogined = false;
	std::atomic_bool settlementinfo_finished = false;
	std::atomic_bool settlementinfo_confirmed = false;
	std::atomic_bool onrspqryinstrument_finished = false;
	std::mutex spi_mutex;
